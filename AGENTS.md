...
- Repository indexes are disposable derived state: canonical files remain authoritative, stale indexes must never be queried, and validation must work without an index. Build indexes in temporary storage with atomic replacement; fingerprint canonical content before and after builds; preserve canonical mutations if refresh fails. Indexed search, filtering, ordering, statistics, eligibility, normalization, and Unicode handling must remain exactly equivalent to canonical fallback. Keep authoritative metadata free of clocks, require `PRAGMA integrity_check` to return exactly `ok`, use ownership-aware build locks, keep approval-critical duplicate checks complete, and never mutate data through the index.
